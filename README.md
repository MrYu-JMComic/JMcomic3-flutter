...
# Dart AOT name shrinking is enabled by default; disable only for local debugging:
.\scripts\build_android_release_split.ps1 -NoObfuscate
```
...
- API/CDN routing settings normalize URL-like input into `host[:port]` before displaying, pinging, or saving; full URLs, protocol-relative URLs, accidentally pasted userinfo, and up to two JSON string layers are stripped to the actual host authority. Host lists also split newline/semicolon-separated values and comma-separated values when the next token is clearly another URL, host, IPv4, bracketed IPv6, or `localhost`, but commas inside query/fragment text stay with the current URL unless the user entered a whitespace-separated explicit URL list. Results dedupe case-insensitively.
