...
class PageDescriptor {
  ...
  PageDescriptor copyWith(
          {bool? isDuplicateName, String? localPath, bool? localAvailable}) =>
      PageDescriptor(
        position: position,
        name: name,
...
class ReaderPageRepository {
  ...
  static List<PageDescriptor> fromOffline(List<DlImage> images) {
    ...
    return _markDuplicates(ordered.asMap().entries.map((entry) {
      ...
      return PageDescriptor(
        ...
        localAvailable: image.localAvailable &&
  ...
  static List<DlImage> mergeLocalAvailability(
    ...
    return List.unmodifiable(base.map((item) {
  ...
  static String _availabilityKey(DlImage image) =>
      '${image.chapterId}\u0000${image.imageIndex}\u0000${image.name}';
