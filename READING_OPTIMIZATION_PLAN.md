...
| M0 基线与工具链冻结 | 待开始（工具链阻塞） | 准备目标 Flutter/Rust 工具链，采集三类设备基线 | 2026-08-30 检查：`flutter`、`dart`、`cargo`、`fvm`、`rustup` 均未找到 |
| M1 低风险稳定性修复 | 待开始 | 先写全屏、排序、生命周期回归测试，再改实现 | 需要明确当前支持的 Flutter API 版本 |
